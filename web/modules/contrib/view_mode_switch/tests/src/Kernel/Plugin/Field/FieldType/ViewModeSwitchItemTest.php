...
class ViewModeSwitchItemTest extends ViewModeSwitchTestBase {
  ...
  /**
   * Tests that 'value' column max length is >= machine name element max length.
   */
  public function testValueMaxLengthCorrespondsToViewModeIdMaxLength(): void {
    ...
    $this->assertArrayHasKey('length', $schema['columns']['value']);

    $element_info_manager = $this->container->get('plugin.manager.element_info');
    $this->assertInstanceOf(ElementInfoManagerInterface::class, $element_info_manager);

    $machine_name_element_info = $element_info_manager->getInfo('machine_name');
    $this->assertArrayHasKey('#maxlength', $machine_name_element_info);

    $this->assertGreaterThanOrEqual($machine_name_element_info['#maxlength'], $schema['columns']['value']['length'], 'Maximum length of view mode switch \'value\' property column is greater than or equal to \'machine_name\' render element\'s maximum length.');
  }
